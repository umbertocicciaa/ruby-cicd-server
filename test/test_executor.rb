...
class MockContainer
  ...
  def initialize(results = {})
    ...
    @default_result = { output: '', stdout: '', stderr: '', status: 0, success: true, execution_time: 0.1,
                        timed_out: false }
    @commands_run = []
  end
...
class TestPipelineValidation < Minitest::Test
  ...
  def test_build_git_clone_command_basic
    ...
    assert_match(%r{'https://github\.com/user/repo\.git'}, cmd)
...
class TestPipelineExecution < Minitest::Test
  ...
  def test_pull_code_success
    ...
    mock = MockContainer.new('git clone' => { output: '', stdout: 'Cloning...', stderr: '', status: 0, success: true,
                                              execution_time: 1.0, timed_out: false })
  ...
  def test_pull_code_failure
    p = Executor::Pipeline.new('https://github.com/user/repo.git', nil, nil)
    mock = MockContainer.new('git clone' => { output: 'fatal', stdout: '', stderr: 'fatal: repo not found',
                                              status: 128, success: false, execution_time: 1.0, timed_out: false })
    p.instance_variable_set(:@container, mock)
  ...
  def test_pull_code_failure_empty_stderr
    p = Executor::Pipeline.new('https://github.com/user/repo.git', nil, nil)
    mock = MockContainer.new('git clone' => { output: 'error from stdout', stdout: 'error from stdout', stderr: '',
                                              status: 1, success: false, execution_time: 0.5, timed_out: false })
    p.instance_variable_set(:@container, mock)
  ...
  def test_pull_code_success_empty_output
    p = Executor::Pipeline.new('https://github.com/user/repo.git', nil, nil)
    mock = MockContainer.new('git clone' => { output: '', stdout: '  ', stderr: '', status: 0, success: true,
                                              execution_time: 0.5, timed_out: false })
    p.instance_variable_set(:@container, mock)
  ...
  def test_run_build_timeout
    ...
    mock = MockContainer.new('long_cmd' => { output: '', stdout: '', stderr: '', status: 124, success: false,
                                             execution_time: 600.0, timed_out: true })
    p.instance_variable_set(:@container, mock)
  ...
  def test_run_build_failure
    ...
    mock = MockContainer.new('bad_build' => { output: 'error', stdout: 'error', stderr: '', status: 1, success: false,
                                              execution_time: 0.5, timed_out: false })
    p.instance_variable_set(:@container, mock)
  ...
  def test_run_deploy_timeout
    ...
    mock = MockContainer.new('slow_deploy' => { output: '', stdout: '', stderr: '', status: 124, success: false,
                                                execution_time: 600.0, timed_out: true })
    p.instance_variable_set(:@container, mock)
  ...
  def test_run_deploy_failure
    ...
    mock = MockContainer.new('bad_deploy' => { output: 'err', stdout: 'err', stderr: '', status: 1, success: false,
                                               execution_time: 0.5, timed_out: false })
    p.instance_variable_set(:@container, mock)
  ...
  def test_execute_build_failure
    ...
    original_with_container = NamespaceSandbox.method(:with_container)
    mock = MockContainer.new('fail_build' => { output: 'err', stdout: 'err', stderr: '', status: 1, success: false,
                                               execution_time: 0.5, timed_out: false })
    NamespaceSandbox.define_singleton_method(:with_container) do |&block|
      block.call(mock)
    end
